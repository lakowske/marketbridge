...
class SessionManager:
    ...
    def get_session_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        """
        Get session metadata by name for API integration.
    ...
    def update_session_metadata(self, name: str, metadata: Dict[str, Any]) -> bool:
        """
        Update session metadata for API integration.

        Args:
            name: Session name to update
        ...
        try:
            session_metadata = self.load_session(name)
            session_metadata.update(metadata)
            session_metadata["last_used"] = datetime.now().isoformat()
            ...
            logger.debug(f"Updated session metadata for: {name}")
            return True

        except Exception as e:
            logger.error(f"Failed to update session metadata for {name}: {e}")
            return False

    def mark_session_active(self, name: str, active: bool = True) -> bool:
        """
        Mark a session as active or inactive for API integration.
        ...
        Returns:
            True if session was updated, False if not found
        """
        return self.update_session_metadata(name, {"active": active})

    def get_active_sessions(self) -> List[Dict[str, Any]]:
        """
        Get all active sessions for API integration.

        Returns:
            List of active session metadata dictionaries
        """
        sessions = []
        for session in self.list_sessions():
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        """
        Get session statistics for API integration.
